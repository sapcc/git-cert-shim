...
build-all: build/git-cert-shim

build/git-cert-shim: FORCE generate
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w $(GO_LDFLAGS)' -o build/git-cert-shim ./cmd/git-cert-shim
...
install: FORCE build/git-cert-shim
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/git-cert-shim "$(DESTDIR)$(PREFIX)/bin/git-cert-shim"
...
generate: install-controller-gen
	...
	@controller-gen crd rbac:roleName=git-cert-shim-role webhook paths="./..." output:crd:artifacts:config=crd output:rbac:artifacts:config=config/rbac
...
goimports: FORCE install-goimports
	@printf "\e[1;36m>> goimports -w -local https://github.com/sapcc/git-cert-shim\e[0m\n"
	@goimports -w -local github.com/sapcc/git-cert-shim $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/git-cert-shim\e[0m          Build git-cert-shim.\n"
